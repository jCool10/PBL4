...
product2vec = pd.read_csv(path + 'product2vec.csv')
...
###################################################################################################################
order_streaks = pd.read_csv('../order_streaks_features.csv')
...
##########################################################################################################################
#判断是否为None单
none_deal=pd.read_csv("stacking_blending.csv")
...
def features(selected_orders, labels_given=False):
    ...
    #df=df.merge(reorder_none_features,on="order_id",how="left")
    #df=df.merge(priors_features,on="order_id",how="left")
    #df = df.join(orders_feature3, on="order_number")
    if labels_given:
