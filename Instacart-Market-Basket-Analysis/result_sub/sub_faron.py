...
if __name__=="__main__":
    f1=F1Optimizer()
    none_pre=pd.read_csv("none_prob.csv")
    data=pd.read_csv("blending_28_12_36_24_shuff.csv")
    data=data[data.label>0.01].copy()  #pre
    ...
    pre=pre.merge(none_pre,on="order_id",how="left")

    pre["best_none"]=map(lambda a,b:f1.maximize_expectation(a,b),pre["label_list"],pre["reorder_num"])
    ...
    sub["products"]=map(lambda a,b,c:play(a,b,c),sub["product_list"],sub["best_num"],sub["if_none"])

    sub=sub[["order_id","products"]]
    sub.to_csv("result_sub_faron_v12_none_blending_28_12_36_24_shuff.csv",index=None)
